...
idle({ask_negotiate, OtherPid}, S=#state{}) ->
    ...
    notice(S, "~p [idle] asked for a trade negotiation", [OtherPid]),
...
idle({negotiate, OtherPid}, From, S=#state{}) ->
    ...
    notice(S, "[idle] asking user ~p for a trade", [OtherPid]),
    Ref = monitor(process, OtherPid),
...
negotiate({make_offer, Item}, S=#state{ownitems=OwnItems, lastPerformer=PerformerPID}) ->
    case PerformerPID == self() of
        ...
        false ->
            ...
            notice(S, "[nego] offering ~p", [Item]),
            {next_state, negotiate, S#state{ownitems=add(Item, OwnItems), lastPerformer=self()}}
    end;
...
negotiate({retract_offer, Item}, S=#state{ownitems=OwnItems, lastPerformer=PerformerPID}) ->
    case PerformerPID == self() of
        ...
        false ->
            ...
            notice(S, "[nego] cancelling offer on ~p", [Item]),
...
wait('ready!', S=#state{}) ->
    ...
    gen_fsm:reply(S#state.from, ok),
    notice(S, "[wait] other side is ready. Moving to ready state", []),
    {next_state, ready, S};
wait(Event, Data) ->
